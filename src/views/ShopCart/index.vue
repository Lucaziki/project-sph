  ...
  <div class="cart">
        ...
        <ul class="cart-list" v-for="item in cartList" :key="item.id">
          ...
          <li class="cart-list-con5">
            ...
            <input
              ...
              minnum="1"
              class="itxt"
              @change="handlerNum('change', $event.target.value * 1, item)"
            />
            <a
              href="javascript:void(0)"
              class="plus"
              @click="handlerNum('add', 1, item)"
              >+</a
            >
          </li>
    ...
    <div class="cart-tool">
      ...
      <div class="money-box">
        ...
        <div class="sumbtn">
          <router-link class="sum-btn" :to="{ name: 'trade' }"
            >结算</router-link
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    handlerNum: throttle(async function (type, disNum, cart) {
      ...
      // 派发action 重新加入购物车
      try {
        await this.$store.dispatch("detailOptions/addShopCart", {
          skuId: cart.skuId,
          skuNum: disNum,
        });
    ...
    async deleteCartById(cart) {
      try {
        await this.$store.dispatch(
          "shopcartOptions/deleteCartListById",
          cart.skuId
        );
        // 删除成功 再发请求获取新的数据
        this.getData();
    ...
    async changeChecked(cart, e) {
      try {
        ...
        await this.$store.dispatch("shopcartOptions/changeCheckedById", {
          skuId: cart.skuId,
          isChecked,
        });
        this.getData();
    ...
    async checkAllCart(e) {
      ...
      try {
        await this.$store.dispatch(
          "shopcartOptions/checkAllIsChecked",
          isChecked
        );
        this.getData();
      } catch (error) {
